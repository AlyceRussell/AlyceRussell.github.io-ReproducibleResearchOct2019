...
The rest of this presentation assumes some knowledge in R. If you are a beginner, however, there will be links to useful resources along the way. 

<br/>

Check out [https://tidyverse.org] for access to the **R for Data Science** eBook as well as working through RStudio workshops, including [https://rmarkdown.rstudio.com] for more on RMarkdown functions.
...
R Markdown documents are **fully reproducible**. You code into the `RMarkdown` document using RStudio, then use `knitr` to weave together narrative text, code and outputs to produce elegantly formatted reports (in Word, PDF, HTML, and it even has formats to produce these slides!!!).
...
RMarkdown files are fully executable so you can be sure there are no mistakes once the file is weaved together (it sends error messages to let you know where issues are within the document). 
...
```{r, echo=FALSE, out.width='40%', fig.align='center', fig.cap=''}
knitr::include_graphics('./images/NameCreateRMarkdown.jpg')
```
...
It is best to store these images within a separate `image` subdirectory in the .Rmd file directory (i.e. where you set up the RProject).
...
* I workshop I gave earlier in the year regarding learning resources in R: [https://youtu.be/GWbgDYRaPr8]
...
2. Open RStudio, create new project (asign to existing directory just created), create new RMarkdown. 

3. Clone [my GitHub respository](https://github.com/AlyceRussell/ReproducibleResearchOct2019). Save the `data` subdirectory, to your new data directory.

4. Edit your RMarkdown document to include a code chunk to 'load dependencies (i.e. `library(tidyverse)`)'.

5. Next 'read in data from the data subdirectory using [**readxl**](https://readxl.tidyverse.org/)', but include `sheet 1` as the main data, and then have `sheet 2` and `sheet 3` loaded in separately. The latter 2 contain value labels. Use `?readxl` to explore function options).

6. Describe your dataset using the `str(x)`, `head(x)`, `names(x)` commands (or similar... note: replace `x` with whatever you called your main dataset).
...
9. Have a look at [**dplyr**](https://dplyr.tidyverse.org/) to see if you can perform any basic manipulations. Maybe counts of species by plot, or weight by species, etc.

10. Have a look at [**ggplot2**](https://ggplot2.tidyverse.org/) and get started on plotting. Also, you can check out my previous workshop here: [http://rpubs.com/r-ladiesperth/datavisualisationbasics]
...
Twitter: [**@nerdyrusty**](www.twitter.com/nerdrusty)

<br/>

R-Ladies Perth: Follow us on [**Meetup**](www.meetup.com/rladies-perth/) or [**Twitter**](www.twitter.com/RLadiesPerth)

<br/>

Slides created via the R packages [**xaringan**](https://github.com/yihui/xaringan), [**knitr**](http://yihui.name/knitr), and [R Markdown](https://rmarkdown.rstudio.com).
